...
public class PinchImageView extends ImageView {
     ...
     * @see #getPinchMode()
     */
    public static int PINCH_MODE_SCALE = 2;
...
//        Log.e("pds", ""+(getDrawable() != null));
//        Log.e("pds", ""+(getDrawable().getIntrinsicWidth() > 0));
//        Log.e("pds", ""+(getDrawable().getIntrinsicHeight() > 0));
//        Log.e("pds", ""+getMeasuredWidth());
//        Log.e("pds", ""+(getMeasuredHeight() > 0));
...
//                PinchImageView.this.getViewTreeObserver().removeGlobalOnLayoutListener(this);
//                width = PinchImageView.this.getWidth();
//                height = PinchImageView.this.getHeight();
//            }
//        });
//        int measuredWidth = getMeasuredWidth();
//        int measuredHeight = getMeasuredHeight();
//        Log.e("pds", width+"measuredWidth:"+measuredWidth);
//        Log.e("pds", width+"measuredHeight:"+measuredHeight);
...
//                    }
                    //在缩放模式下移动
                } else if (mPinchMode == PINCH_MODE_SCALE && event.getPointerCount() > 1) {//双指缩放
//                } else if (mPinchMode == PINCH_MODE_SCALE) {//双指缩放
                    //两个缩放点间的距离
                    ...
                    //处理缩放
                    scale(mScaleCenter, mScaleBase, distance);
                }
            }
        }
        //无论如何都处理各种外部手势
...
//        Log.e("pds", displayWidth+"  ::bound width: "+bound.width()+ "  "+bound.height()+ " bound height  :: right:"+bound.right+"  ::  left:"+bound.left + "  top"+bound.top + "   bottom:"+bound.bottom + "   centerX:"+bound.centerX());
//        // 如果图片的宽度大于控件的宽度
//        Log.e("pds", "x:"+xDiff + "  :");
        if(bound.centerX() > displayWidth){
            if(xDiff > 0)
                xDiff = 0;
        ...
        if (xDiff != 0 || yDiff != 0) {
           /* if (getContext() instanceof BaiBanCheckImageActivity) {
                Log.e("pds", "xDiff:"+xDiff + " yDiff:"+yDiff);
                if (((BaiBanCheckImageActivity) getContext()).rightBarAdapter != null && ((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isSpeaker) {
                    int diffX = (int) (IsDownCache.getInstance().getInt("BaibanCenterX") - xDiff);
                    int diffY = (int) (IsDownCache.getInstance().getInt("BaibanCenterY") - yDiff);
                    Log.e("pds", "diffX:"+diffX + " diffY:"+diffY);
                    float nLeftX = bound.left < 0 ? -bound.left:0;//负数取反  正数恒为0
                    float nLeftY = bound.top < 0 ? -bound.top:0;//图片原点Y轴的绝对值
                    float nRightX;
                    if(bound.right < getWidth()){//图片右下角的x轴不超过屏幕,  则接收的值为图片放大后的宽度
                        nRightX = bound.width();
                    }else{//图片右下角的x轴超过屏幕,  则用图片放大后的宽度减去超出屏幕的宽度
                        nRightX = bound.width() - (bound.right - getWidth());
                    }
                    float nRightY = getHeight() > bound.bottom ? bound.height() : bound.height() - (bound.bottom - getHeight());// 图片底部未超出屏幕, 则传图片的高度,  超出屏幕, 则传 图片高度减去超出的高度
                    ((BaiBanCheckImageActivity) getContext()).sendRightBarZhujiangData(0, 1, diffX, diffY, ConfigUtil.MOVE_CLICK, getMaxScale(),(int)bound.centerX(), (int)bound.centerY(), new PointF(nLeftX, nLeftY), new PointF(nRightX, nRightY), bound.width(), bound.height());
                    if (!((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isClickLiulan) {
                        Log.e("pds", "白板页移动  保存数据");
                        IsDownCache.getInstance().putInt("BaibanCenterX", diffX);
                        IsDownCache.getInstance().putInt("BaibanCenterY", diffY);
                    }
                }
            }*/
            return true;
    ...
    public void doubleTapTest(float x, float y){
        if (!isReady()) {
            return;
        }
        RectF bound = MathUtils.rectFTake();
...
//按照图片缩放中心缩放，并且让缩放中心在缩放点中点上
        ...
        //原图方框
        Log.e("pds", "  ::  "+  "    width:"+bound.width()+"  ::  left:"+bound.left + "    "+getHeight());
        if(bound.width()*2 < getWidth()){//如果当前图片宽度小于控件宽度 ，则把点击x重置到中点
            x = getWidth() / 2;
        }
        Log.e("pds", "x:"+x + "y:"+y);
        if(y < (getHeight() * 0.2f)){
        ...
        innerMatrix.postScale(scale, scale, x, y);
        //清理当前可能正在执行的动画
        getBitmapPoint(innerMatrix);
        cancelAllAnimator();
        //启动矩阵动画
        if(scale == 1){
            mScaleAnimator = new ScaleAnimator(mOuterMatrix, innerMatrix);
        }else{
            mScaleAnimator = new ScaleAnimator(MathUtils.matrixTake(), innerMatrix);
        }
        mOuterMatrix.set(innerMatrix);
        mScaleAnimator.start();
        //应用变换
        ...
        Log.e("pds", "bound.width(():"+bound.width() + "   bound.height():"+ bound.height());
       /* if (getContext() instanceof BaiBanCheckImageActivity) {
            if (((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isSpeaker) {
                float nLeftX = bound.left < 0 ? -bound.left:0;//负数取反  正数恒为0
                float nLeftY = bound.top < 0 ? Math.abs(bound.top):0;
                float nRightX;
                if(bound.right < getWidth()){//图片右下角的x轴不超过屏幕,  则接收的值为图片放大后的宽度
                    nRightX = bound.width();
                }else{//图片右下角的x轴超过屏幕,  则用图片放大后的宽度减去超出屏幕的宽度
                ...
                float nRightY = getHeight() > bound.bottom ? bound.height() : bound.height() - (bound.bottom - getHeight());// 图片底部未超出屏幕, 则传图片的高度,  超出屏幕, 则传 图片高度减去超出的高度
                ((BaiBanCheckImageActivity) getContext()).sendRightBarZhujiangData(0, 1, (int) x, (int) y, ConfigUtil.DOUBLE_CLICK, scale, (int)bitmapPointX, (int)bitmapPointY, new PointF(nLeftX, nLeftY), new PointF(nRightX, nRightY), bound.width(), bound.height());
                if (!((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isClickLiulan) {
                    IsDownCache.getInstance().putInt("BaibanCenterX", (int) x);
                    IsDownCache.getInstance().putInt("BaibanCenterY", (int) y);
                }
                IsDownCache.getInstance().putFloat("BaibanCenterScale", scale);
            }
        }*/
    }
    /**
     * 得到移动放大后的图片相对于屏幕的原点
     */
    private void getBitmapPoint(Matrix innerMatrix){
        // 存储Matrix矩阵的9个值
        float[] matrixValues = new float[9];
        innerMatrix.getValues(matrixValues);
        // 变化的倍数
        ...
        float y=0;
        // 变化后的点
        bitmapPointX = (x * mscale_x + 1 * mtrans_x + getWidth());
        bitmapPointY = (y * mscale_y + 1 * mtrans_y + getHeight());
    }
    private float bitmapPointX;
    private float bitmapPointY;
    public void doubleTapTest(float x, float y, float scale){
        if (!isReady()) {
            return;
        }
        this.scale = scale;
     ...
     * @see #mScaleBase
     */
    public void scale(PointF scaleCenter, float scaleBase, float distance) {
        if (!isReady()) {
            return;
        }
        ...
        float nLeftX = bound.left < 0 ? -bound.left:0;//负数取反  正数恒为0
        float nLeftY = bound.top < 0 ? Math.abs(bound.top):0;//图片原点Y轴的绝对值
        float nRightX;
        if(bound.right < getWidth()){//图片右下角的x轴不超过屏幕,  则接收的值为图片放大后的宽度
            nRightX = bound.width();
        }else{//图片右下角的x轴超过屏幕,  则用图片放大后的宽度减去超出屏幕的宽度
       ...
       /* if (getContext() instanceof BaiBanCheckImageActivity) {
            if (((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isSpeaker) {
                ((BaiBanCheckImageActivity) getContext()).sendRightBarZhujiangData(0, 1, (int) scaleCenter.x, (int) scaleCenter.y, ConfigUtil.TWO_CLICK, scale, (int)bound.centerX(), (int)bound.centerY(), new PointF(nLeftX, nLeftY), new PointF(nRightX, nRightY), bound.width(), bound.height());
                if (!((BaiBanCheckImageActivity) getContext()).rightBarAdapter.isClickLiulan) {
                    IsDownCache.getInstance().putInt("BaibanCenterX", (int) scaleCenter.x);
                    IsDownCache.getInstance().putInt("BaibanCenterY", (int) scaleCenter.y);
                }
                IsDownCache.getInstance().putFloat("BaibanCenterScale", scale);
            }
        }*/
